		...
		try
		{
			if (leap_hand != null)
			{
				...
				if (leap_hand.IsPinching())
				{
					...
					//disegno sulla tela
					tex = DrawCircle(tex, color, (int)(tex.width * (finger.GetTipPosition().x + 0.5)), (int)(tex.height * (finger.GetTipPosition().y + 0.5)), brushSize);
					//Debug.Log((int)(tex.height * (finger.GetTipPosition().y + 0.5)));
					tex.Apply();
				}
